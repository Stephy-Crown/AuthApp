...
export async function POST(req) {
  await connect(); // Ensure connection to MongoDB at the start

  const SIGNING_SECRET = process.env.SIGNING_SECRET;
  if (!SIGNING_SECRET) {
    throw new Error(
      "Error: SIGNING_SECRET is not set in environment variables"
    );
  }
  ...
  return new Response("Webhook event received", { status: 200 });
}
